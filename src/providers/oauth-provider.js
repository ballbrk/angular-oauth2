...
function OAuthProvider() {
    ...
    this.$get = function ($http, OAuthToken) {
        class OAuth {
            ...
            /**
             * Register a user from Facebook connect to server.
             * @return {promise} A response promise.
             */
            convertFacebookToken(token,options) {
                var data = {
                    client_id: config.clientId,
                ...
                return $http.post(`${config.baseUrl}${config.convertTokenPath}`, data, options).then((response) => {
                    OAuthToken.setToken(response.data);
